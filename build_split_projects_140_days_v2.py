...
STUDENTS = [
    ...
    ("BE2", "Rafli Dika Rendra Arifin", "1402023061", "LOGBOOK_BE2_RAFLI_DIKA_RENDRA_ARIFIN", "DAFTAR_HADIR_BE2_RAFLI_DIKA_RENDRA_ARIFIN"),
    ("FE1", "Aldi Afriansyah", "1402023062", "LOGBOOK_FE1_ALDI_AFRIANSYAH", "DAFTAR_HADIR_FE1_ALDI_AFRIANSYAH"),
    ("FE2", "Farras Fadhillah", "1402023063", "LOGBOOK_FE2_FARRAS_FADHILLAH", "DAFTAR_HADIR_FE2_FARRAS_FADHILLAH")
]

# ALSO SAVE GENERIC ALIASES (LOGBOOK_BE2.docx etc.) FOR COMPATIBILITY
STUDENT_ALIASES = [
    ("BE2", "LOGBOOK_BE2", "DAFTAR_HADIR_BE2"),
    ("FE1", "LOGBOOK_FE1", "DAFTAR_HADIR_FE1"),
    ("FE2", "LOGBOOK_FE2", "DAFTAR_HADIR_FE2")
]
...
# --- 2. GENERATE LOGBOOK FILES (2 PROJECTS & REAL NAMES) ---
def generate_logbook_files(role, name, npm, logbook_fname):
    ...
    # Markdown
    md_content = f"# LOGBOOK LAPORAN KEGIATAN MAGANG ({role} - {name})\n"
...
<body>
<div class="container">
    <h1>LOGBOOK LAPORAN KEGIATAN MAGANG — {name.upper()} ({role})</h1>
    <div class="info">
...
"""
    ...
    p = doc.add_paragraph()
    r = p.add_run(f"LOGBOOK LAPORAN KEGIATAN MAGANG — {name.upper()} ({role})\n")
    r.bold = True
    r.font.size = Pt(15)
    ...
    doc.save(docx_path)
    print(f"Generated Logbook ({name}): {docx_path}")

# --- 3. GENERATE DAFTAR HADIR FILES (REAL NAMES) ---
...
"""
    ...
    doc.save(docx_path)
    print(f"Generated Hadir DOCX ({name}): {docx_path}")
...
if __name__ == '__main__':
    ...
    # ALSO GENERATE ALIAS FILES (LOGBOOK_BE2, LOGBOOK_FE1, LOGBOOK_FE2) FOR EASY ACCESS
    for role, name, npm, _, _ in STUDENTS:
        if role == "BE2":
            generate_logbook_files(role, name, npm, "LOGBOOK_BE2")
            generate_hadir_files(role, name, npm, "DAFTAR_HADIR_BE2")
        elif role == "FE1":
            generate_logbook_files(role, name, npm, "LOGBOOK_FE1")
            generate_hadir_files(role, name, npm, "DAFTAR_HADIR_FE1")
        elif role == "FE2":
            generate_logbook_files(role, name, npm, "LOGBOOK_FE2")
            generate_hadir_files(role, name, npm, "DAFTAR_HADIR_FE2")
