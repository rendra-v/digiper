...
@section('title', 'Rekap Kepaniteraan & Tim')

@section('content')
@php
    function rp_kpt(int $v): string {
...
<div class="min-h-screen">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h2 class="text-4xl font-semibold tracking-tight mb-3">Rekap Kepaniteraan &amp; Tim</h2>
            <p class="text-neutral-500 dark:text-neutral-400">
                Rekapitulasi honorarium biaya penyelesaian perkara per jenis
                @if($fileName)
        ...
        <div class="flex items-center gap-3">
            ...
            @if(!$error && !empty($blocks))
            <a href="{{ route('rekap-kepaniteraan-tim.print') }}" target="_blank" rel="noopener"
    ...
    @else
        ...
        <div class="mb-6 flex gap-2 border-b border-neutral-200 dark:border-neutral-700">
            ...
            <a href="{{ route('rekap-kepaniteraan-tim', ['kamar' => $km]) }}"
        ...
        @php
            $filteredBlocks = collect($blocks)->filter(fn($b) => $b['kamar'] === $activeKamar)->values();
        @endphp

        @if($filteredBlocks->isEmpty())
            <p class="text-center text-neutral-400 text-sm py-8">Tidak ada data untuk kamar {{ $activeKamar }}.</p>
        @else
            @foreach($filteredBlocks as $bi => $block)
            @php
                $originalIdx = collect($blocks)->search(fn($b) => $b['title'] === $block['title']);
            @endphp
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-800/40">
                    <div>
                        ...
                        <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                            Pada Kamar {{ $block['kamar'] }} &mdash; {{ $block['jml_perkara'] }} Perkara
                        </p>
                    </div>
                    <a href="{{ route('rekap-kepaniteraan-tim.print', ['block' => $originalIdx]) }}"
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                            ...
                            <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors">
                                ...
                                <td class="px-4 py-3 font-medium text-neutral-800 dark:text-neutral-100">{{ $row['nama'] }}</td>
                                <td class="px-4 py-3 text-right font-mono text-neutral-700 dark:text-neutral-300">
                                    {!! $row['jumlah_biaya'] > 0 ? rp_kpt($row['jumlah_biaya']) : '<span class="text-neutral-400">Rp&nbsp;-</span>' !!}
                                </td>
                                <td class="px-4 py-3 text-right font-mono text-neutral-700 dark:text-neutral-300">
                                    {!! $row['pph15'] > 0 ? rp_kpt($row['pph15']) : '<span class="text-neutral-400">Rp&nbsp;-</span>' !!}
                                </td>
                                <td class="px-4 py-3 text-right font-mono text-neutral-700 dark:text-neutral-300">
                                    {!! $row['pph5'] > 0 ? rp_kpt($row['pph5']) : '<span class="text-neutral-400">Rp&nbsp;-</span>' !!}
                                </td>
                                <td class="px-4 py-3 text-right font-mono text-neutral-700 dark:text-neutral-300">
                                    {!! $row['netto'] > 0 ? rp_kpt($row['netto']) : '<span class="text-neutral-400">Rp&nbsp;-</span>' !!}
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
                            <tr class="bg-neutral-100 dark:bg-neutral-800 font-bold text-sm">
                                ...
                                <td class="px-4 py-3 text-right font-mono">
                                    {!! $block['total']['jumlah_biaya'] > 0 ? rp_kpt($block['total']['jumlah_biaya']) : 'Rp&nbsp;-' !!}
                                </td>
                                <td class="px-4 py-3 text-right font-mono">
                                    {!! $block['total']['pph15'] > 0 ? rp_kpt($block['total']['pph15']) : 'Rp&nbsp;-' !!}
                                </td>
                                <td class="px-4 py-3 text-right font-mono">
                                    {!! $block['total']['pph5'] > 0 ? rp_kpt($block['total']['pph5']) : 'Rp&nbsp;-' !!}
                                </td>
                                <td class="px-4 py-3 text-right font-mono">
                                    {!! $block['total']['netto'] > 0 ? rp_kpt($block['total']['netto']) : 'Rp&nbsp;-' !!}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
